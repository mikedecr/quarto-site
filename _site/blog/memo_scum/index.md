# Scraping the ‘S.C.U.M.’ data from ‘Kill James Bond’
Michael DeCrescenzo
2024-03-02
...
## A *Kill James Bond* and the S.C.U.M. scores
...
When we start our journey, the website is a black box. I know what I
...
values. I want a function that would take me to each individual value…

<figure class=''>

<img src="index_files/figure-markdown_strict/mermaid-figure-1.png"
style="width:2.6in;height:3.13in" />
...
And of course, I should be a be able to generate an list of all movies
...
-   Starting with the global site url, I can generate a sequence of film
    urls.
-   I write a function (call it `get_movie_data`) that takes a film url
    and generates a tuple or a “record” of SCUM scores and other
    metadata.
...
Here I show a first-cut at the interface. Each function has to do some
...
4.2 seconds per film? There are 27 films to get through. It’s not a
...
present, so we would only download the html code one time per url. This
idea has some advantages, but it would require us to create 27 class
instances from all of our urls. In fairness, each of these objects could
hold the url as an attribute, so it’s still just 27 objects, I don’t see
any point to this kind of object-oriented complexity. I *already have
the data I need*, which is the url damnit, and I *already have* the
functions I need call on the url. I dont’ want to bother with writing a
class when I could keep the elegant `url --> value` interface pattern.

So instead: we **memoize** the `read_html` function. This gives us the
same performance boost as the other methods (we download the html code
just one time), but it does not compromise the interface interactions in
any way. Really.
...
-   A pure function is “mathematically” indistinguishable from a lookup
    table. If you give a function an input, it gives you its output, the
    same way every time.
-   The first time a function is evaluated on a set of arguments, the
    returned value is *stored in a hidden lookup table* that the
    function holds onto and can consult whenever it is called.
-   If the function is ever called with the same argument more than
    once, it can return the *previously computed value* from the table
    instead of recalculating the value from scratch again.

This technique is valuable when the function is expensive and will be
...
The first time I call `memo_double` with argument `4`, the function will
...
0.348 seconds, a 12x speedup in this example.
...
![](index.markdown_strict_files/figure-markdown_strict/unnamed-chunk-23-1.png)
...
`functools.cache`. But it only works with hashable function arguments.
Unfortunately a lot of Python is made of unhashable data, like lists,
sets, dictionaries, Numpy arrays, and Pandas dataframes, which makes
memoization challenging. Furthermore, almost all data in Python is
mutable, so if a memoized function returns a mutable object, and you
mutate it later, then the memoized function can return something
different from what you expect. R has a third-party package called
`memoise` that gives you a `memoise::memoise` function. I am not as
familiar with caching in R, but because R data is mostly immutable (R
...
Would that honestly be so bad? `juxt` deserves some justice is all I’m
saying.
